...
def main():
    ...
    after_reset = time.time() - start_time

    # Now recommend items to users.
    print("Making online recommendations")
    loop_times = np.zeros(100)
    for i in range(100):
        loop_start = time.time()
        online_users = env.online_users()
        ret, predicted_ratings = recommender.recommend(online_users, num_recommendations=5, strategy="thompson")
