...
def create_user_index(ui_mat_rdd,
	...
	user_index = ui_mat_rdd.map(lambda usrId: usrId) \
		.distinct() \
		.zipWithIndex() \
		.collect()  # user_index is a list here
	print ('check create_user_index function')
	print (len(user_index), type(user_index))
	for test in user_index[:1]:
		#((1, 661, 3.0), 0)  ((UserID,MovieID,Rating), index)
		#(1, 661, 3.0) is a tuple
		test2 = list(zip([test[0]], [test[1]]))
		print ('test2', test2)

	user_index = dict(zip([usr[0] for usr in user_index],[usr[1] for usr in user_index]))
	print ('check create_user_index function again')
	print (len(user_index), type(user_index))
	

	if export:
		with open('Users/Guo/Documents/Data/ml-1m/user_index.json', 'w') as fp:
...
def create_doc_index(ui_mat_rdd,
	...
	doc_index = ui_mat_rdd.map(lambda docId: docId) \
		.distinct() \
		.zipWithIndex() \
		.collect()
	...
	if export:
		with open('Users/Guo/Documents/Data/ml-1m/doc_index.json', 'w') as fp:
...
def load_indexes():
	"""
	Load the user and document indexes used for the model. Here the indexes are also
	reversed in order to be used for the reverse mapping.
	Returns:
	...
	with open('/local/agidiotis/ui_data/doc_index.json') as json_file:
		json_string = json_file.read()
		doc_index = json.loads(json_string)
		doc_index = dict([int(k), int(v)] for k,v in doc_index.items())

	with open('/local/agidiotis/ui_data/user_index.json') as json_file:
		json_string = json_file.read()
